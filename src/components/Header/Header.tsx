...
export default function Header({ onSettingsClick, onHomeClick }: HeaderProps) {
  ...
  return (
    <header className="border-b border-white/[0.04] bg-stone-900/86 backdrop-blur-sm">
      <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-3 px-4 py-3 sm:px-6">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="rounded-xl border border-white/[0.04] bg-white/[0.03] p-2.5 text-stone-300 transition-colors hover:bg-white/[0.05] hover:text-white cursor-pointer"
            title="Settings"
...
function PhaseBadge({ phase }: { phase: string }) {
  const config: Record<string, { label: string; color: string }> = {
    idle: { label: 'Idle', color: 'border-white/[0.04] bg-white/[0.03] text-stone-400' },
